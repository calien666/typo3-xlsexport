...
#
# calien/xlsexport test runner based on docker/podman.
#
if [ "${CI}" != "true" ]; then
...
loadHelp() {
    ...
    read -r -d '' HELP <<EOF
xlsexport test runner. Execute unit, functional and other test suites in a container
based test environment. Handles execution of single test files, sending xdebug
...
Options:
    ...
    -i version
        ...
        With "-d mariadb": 10.4 (default) 10.5 10.6 10.7 10.8 10.9 10.10 10.11 11.0 11.1
        With "-d mysql":   8.0 (default) 8.1 8.2 8.3 8.4
        With "-d postgres": 10 (default) 11 12 13 14 15 16
...
Examples:
    ...
    # Run functional tests on TYPO3 v14 with PHP 8.5 on mariadb
    ./Build/Scripts/runTests.sh -s functional -t 14 -p 8.5 -d mariadb

    # Run phpstan against the TYPO3 v14 configuration
    ./Build/Scripts/runTests.sh -s phpstan -t 14
EOF
}
...
case ${TEST_SUITE} in
    ...
    checkBom)
        ...
        ${CONTAINER_BIN} run ${CONTAINER_COMMON_PARAMS} --name check-bom-${SUFFIX} -e COMPOSER_CACHE_DIR=.Build/.cache/composer -e COMPOSER_ROOT_VERSION=${COMPOSER_ROOT_VERSION} ${IMAGE_PHP} /bin/sh -c "${COMMAND}"
        SUITE_EXIT_CODE=$?
        ;;
    checkRst)
        COMMAND="php -dxdebug.mode=off Build/Scripts/validateRstFiles.php"
        ${CONTAINER_BIN} run ${CONTAINER_COMMON_PARAMS} --name check-rst-${SUFFIX} -e COMPOSER_CACHE_DIR=.Build/.cache/composer -e COMPOSER_ROOT_VERSION=${COMPOSER_ROOT_VERSION} ${IMAGE_PHP} /bin/sh -c "${COMMAND}"
        SUITE_EXIT_CODE=$?
        ;;
    checkExceptionCodes)
        COMMAND="Build/Scripts/duplicateExceptionCodeCheck.sh"
        ${CONTAINER_BIN} run ${CONTAINER_COMMON_PARAMS} --name check-exception-codes-${SUFFIX} -e COMPOSER_CACHE_DIR=.Build/.cache/composer -e COMPOSER_ROOT_VERSION=${COMPOSER_ROOT_VERSION} ${IMAGE_PHP} /bin/sh -c "${COMMAND}"
        SUITE_EXIT_CODE=$?
        ;;
    checkIntegrityXliff)
        COMMAND="php Build/Scripts/checkIntegrityXliff.php"
        ${CONTAINER_BIN} run ${CONTAINER_COMMON_PARAMS} --name check-xliff-${SUFFIX} -e COMPOSER_CACHE_DIR=.Build/.cache/composer -e COMPOSER_ROOT_VERSION=${COMPOSER_ROOT_VERSION} ${IMAGE_PHP} /bin/sh -c "${COMMAND}"
        SUITE_EXIT_CODE=$?
        ;;
    checkTestMethodsPrefix)
        COMMAND="php -dxdebug.mode=off Build/Scripts/testMethodPrefixChecker.php"
        ${CONTAINER_BIN} run ${CONTAINER_COMMON_PARAMS} --name check-test-prefix-${SUFFIX} -e COMPOSER_CACHE_DIR=.Build/.cache/composer -e COMPOSER_ROOT_VERSION=${COMPOSER_ROOT_VERSION} ${IMAGE_PHP} /bin/sh -c "${COMMAND}"
        SUITE_EXIT_CODE=$?
        ;;
    clean)
        cleanCacheFiles
    ...
    functional)
        ...
        ;;
    lintPhp)
        COMMAND="find . -name \\*.php ! -path "./.Build/\\*" ! -path "./.cache/\\*" ! -path "./Resources/Private/Php/\\*" -print0 | xargs -0 -n1 -P4 php -dxdebug.mode=off -l >/dev/null"
    ...
    renderDocumentation)
        ...
        if [ -d "${ROOT_DIR}/Documentation" ]; then
            ${CONTAINER_BIN} run ${CONTAINER_INTERACTIVE} --pull always -v ${ROOT_DIR}:/project -it ${IMAGE_DOCS} --config=Documentation --no-progress --fail-on-log
            SUITE_EXIT_CODE=$?
        else
            echo "No Documentation/ directory found, nothing to render."
        fi
        ;;
    phpstan)
    ...
    unit)
        ...
        # separates project path (always the real root) from public path (.Build/Web, this
        # project's composer.json web-dir).
